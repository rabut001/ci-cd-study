...
>
> - **PR マージ前の自動ゲートとしてのスキーマ検証**は、Preview ではなく **フェーズ4の `e2e` ジョブ（方式A：ランナー内 Supabase に PR の migration を適用）** で担保する（こちらはコードとスキーマが常に一致する）。
...
> - 当面は「**スキーマ検証は方式Aの E2E、Preview は UI 目視確認**」と役割を分けると、無料のまま安全に回せる。
