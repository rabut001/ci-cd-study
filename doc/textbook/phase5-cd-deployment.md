...
> Preset 空のまま Deploy すると、ビルドは成功しても Production → Visitで **404 NOT_FOUND** になり得る。ステップ 2 は必須。
...
| **Ready** だが **404 NOT_FOUND** | ステップ 2 の **Framework Preset = Next.js** を確認 |
...
git commit -m "feat: change page heading to ToDo List(ToDo Ichiran)"
...
| **フェーズ6** | ブランチ → PR → Preview → マージ → 本番 の **一連の実演**（ToDo に期限を追加）＋ **CI の最適化**（§11） |
